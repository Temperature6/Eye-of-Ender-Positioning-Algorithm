...
FLAG:
	...
	for (int i = 0; i < 3; i++)
	{
		...
		scanf_s("%f %f %f", &throw_point[i][0], &throw_point[i][1], &throw_point[i][2]);
		system("cls");
	}
	...
	float _X = (max(x_arr, len(x_arr)) + min(x_arr, len(x_arr))) / 2;
	float _Z = (max(z_arr, len(z_arr)) + min(z_arr, len(z_arr))) / 2;
